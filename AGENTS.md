...
**Stub 型別保留（已放寬自給自足）**：generator（`make_self_contained.py`）保留所有有 deployed `.pyi` 的 module 的 cross-package 型別（不限 same-package）——還原繼承 + 精確回傳（`Cache(CacheFacade)`、`order() -> Order | None` 等）。無 `.pyi` 的依賴仍用 `Any`。代價：stub 間 cross-package 互依，靠 export-filtering + 整批部署（`make n`）控制。詳見 `scripts/lsp_stubs/README.md`。
...
僅列出 mosaic_alpha 實際使用或有參考價值的模組。詳細 AGENTS.md 見各模組目錄。

### 核心交易

- `trading/` → [AGENTS.md](nautilus_trader/trading/AGENTS.md) — Strategy base class、Trader orchestrator
- `model/` → [AGENTS.md](nautilus_trader/model/AGENTS.md) — Fixed-point types、identifiers、instruments、orders
- `backtest/` → [AGENTS.md](nautilus_trader/backtest/AGENTS.md) — BacktestEngine、FeeModel/FillModel
- `execution/` → [AGENTS.md](nautilus_trader/execution/AGENTS.md) — ExecutionEngine、order FSM、exec algorithms
- `risk/` → [AGENTS.md](nautilus_trader/risk/AGENTS.md) — Pre-trade risk engine
- `portfolio/` → [AGENTS.md](nautilus_trader/portfolio/AGENTS.md) — Position tracking、account balances
- `live/` → [AGENTS.md](nautilus_trader/live/AGENTS.md) — TradingNode、adapter wiring、reconciliation
- `accounting/` → [AGENTS.md](nautilus_trader/accounting/AGENTS.md) — Cash/Margin/Betting accounts

### 基礎設施

- `common/` → [AGENTS.md](nautilus_trader/common/AGENTS.md) — Component FSM、Actor、MessageBus、OrderFactory
- `cache/` → [AGENTS.md](nautilus_trader/cache/AGENTS.md) — In-memory cache、Redis adapter
- `data/` → [AGENTS.md](nautilus_trader/data/AGENTS.md) — Data engine、bar aggregation、subscriptions
- `config/` → [AGENTS.md](nautilus_trader/config/AGENTS.md) — Frozen msgspec dataclass configs
- `system/` → [AGENTS.md](nautilus_trader/system/AGENTS.md) — Kernel assembly、database wiring

### 資料與分析

- `indicators/` → [AGENTS.md](nautilus_trader/indicators/AGENTS.md) — Technical indicators（純 Cython；Rust crate migration 中）
- `persistence/` → [AGENTS.md](nautilus_trader/persistence/AGENTS.md) — ParquetDataCatalog、wranglers
- `analysis/` → [AGENTS.md](nautilus_trader/analysis/AGENTS.md) — PortfolioAnalyzer、tearsheet
